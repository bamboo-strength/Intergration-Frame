...
const PropTypes = {
  ...
  custom(validatorFn: any, warnMsg = 'custom validation failed') {
  ...
  oneOf(arr: any[]) {
    ...
    const allowedTypes = arr.reduce((ret: any[], v: { constructor: any; } | null | undefined) => {
  ...
  oneOfType(arr: any[]) {
    ...
    const nativeChecks = arr.reduce((ret: any[], type: { _vueTypes_name: string; type: any; validator: any; }) => {
    ...
    const typesStr = arr
      ...
      .reduce((ret: any, type: any) => ret.concat(isArray(type) ? type : [type]), [])
      .join('", "');

    return this.custom(function oneOfType(value: any) {
      const valid = arr.some((type: { _vueTypes_name: string; type: any; }) => {
        if (type._vueTypes_name === 'oneOf') {
  ...
  arrayOf(type: any) {
    return toType('arrayOf', {
      ...
      validator(values: any[]) {
        const valid = values.every((value: any) => validateType(type, value));
